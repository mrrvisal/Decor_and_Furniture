...
class Router
{
    ...
    private function getUri(): string
    {
        ...
        // Strip /index.php suffix when mod_rewrite is off
        if (preg_match('#^(.*)/index\.php/*$#', $uri, $m)) {
            $uri = $m[1] === '' ? '/' : $m[1];
        }
        // Strip the basePath prefix so routes are matched relative to the app root
        // e.g. /php/Decor_and_Furniture/public/products -> /products
